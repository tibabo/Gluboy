...
#define reg_f  (*((unsigned char*)&af))
#define reg_a  (*(((unsigned char*)&af) + 1))
...
#define update_carry() {flag.H = (carrybits & 0x10)>0; flag.C = (carrybits & 0x100)>0;}

#define INC_ss(a) { [](){ TC += 8;  a++;  PC +=1;}, "INC " #a  }
#define INC_r(a,clock) { [](){ TC += clock;  flag.N = 0; flag.H = (a & 0x0F) == 0x0F; a += 1;  flag.Z = (a == 0);  PC +=1;}, "INC " #a }

#define DEC_r(a,clock) { [](){ TC += clock;  flag.N = 1; flag.H = (a & 0x0F) == 0x00; a -= 1;  flag.Z = (a == 0);  PC +=1;}, "DEC " #a }
...
#define LD_ssp_r(a,r) {[](){ TC += 8;  ram[a] = r;                                                                 PC +=1;}, "LD (" #a "), A" }
#define ADD_ss_ss(a,b) {[](){ TC += 8;  flag.N = 0; uint32_t sum = a + b; flag.H = (sum & 0xFFFF) ^ a ^ b ^ 0x1000;  a = sum; flag.C = sum & 0x10000; PC +=1;}, "ADD " #a ", " #b } 
#define DEC_ss(a) { [](){ TC += 8;  a--;                                                               PC +=1;}, "DEC " #a }
#define LD_r_ss(r,ss) { [](){ TC += 8; r = ram[ss];                                                     PC +=1;}, "LD " #r ", (" #ss ")" }
#define LD_r_r(a,b) { [](){ TC += 4; a = b;                                                             PC +=1;}, "LD " #a ", " #b }
#define SUB_r(x,clock) {  [](){ TC += clock;  flag.N = 1; int result = reg_a - x; int carrybits = x ^ reg_a ^ result; reg_a = result; update_carry(); flag.Z = (reg_a == 0); PC +=1;}, "SUB " #x }

#define ADD_r(x,clock)  { [](){ TC += clock;  flag.N = 0; int result = reg_a + x; int carrybits = x ^ reg_a ^ result; reg_a = result; update_carry(); flag.Z = (reg_a == 0); PC +=1;}, "ADD A, " #x }

#define AND_r(x,clock)  { [](){ TC += clock;  flag.N = 0; int result = reg_a & x; reg_a = result; flag.H = 1; flag.C = 0; flag.Z = (reg_a == 0); PC +=1;}, "AND A, " #x }
#define OR_r(x,clock)   { [](){ TC += clock;  flag.N = 0; int result = reg_a | x; reg_a = result; flag.H = 0; flag.C = 0; flag.Z = (reg_a == 0); PC +=1;}, "OR A, " #x }
#define ADC_r(x,clock)  { [](){ TC += clock;  flag.N = 0; int result = reg_a + x + flag.C; flag.H = ((reg_a & 0x0F) + (x & 0x0F) + flag.C ) > 0x0F; flag.C = result > 0xFF; reg_a = result; flag.Z = (reg_a == 0); PC +=1;}, "ADC A, " #x }
#define SBC_r(x,clock)  { [](){ TC += clock;  flag.N = 1; int result = reg_a - x - flag.C; int carrybits = x ^ reg_a ^ result ^ flag.C; update_carry(); reg_a = result; flag.Z = (reg_a == 0); PC +=1;}, "SBC A, " #x }
#define XOR_r(x,clock)  { [](){ TC += clock;  flag.N = 0; int result = reg_a ^ x; reg_a = result; flag.H = 0; flag.C = 0; flag.Z = (reg_a == 0); PC +=1;}, "XOR A, " #x }
#define CP_r(x,clock)   { [](){ TC += clock;  flag.N = 1; int result = reg_a - x; int carrybits = x ^ reg_a ^ result; update_carry(); flag.Z = (reg_a == 0); PC +=1;}, "CP A, " #x }
...
#define CALL_X_A16(r)   { [](){ if(r) {TC+=24; int destination = rom[PC + 1] | (rom[PC + 2]<<8); PC+=3; ram[--SP] = PC; ram[--SP] = PC>>8; PC = destination;} else { TC+=12; PC+=3; }},  "CALL " #r ", 0x%02x%02x"}
#define PUSH_ss(r)      { [](){ TC+=16; ram[--SP] = r; ram[--SP] = r>>8; }, "PUSH " #r }
#define RET_X(r)        { [](){ if(r) {TC+=20; PC = ram[SP] | (ram[SP+1]<<8); SP+=2;} else {TC+=8; PC+=1; } }, "RET " #r }
#define POP_ss(r)       { [](){ TC+=12; r = ram[SP] | (ram[SP+1]<<8); SP+=2; }, "POP " #r }

#define RST_n(r)        { [](){ TC+=16; PC += 1; ram[--SP] = PC; ram[--SP] = PC>>8; PC = r; }, "RST " #r}
#define RLC_n(r)        { [](){ TC += 8; char carry = (r & 0x80)>>7; r = (r << 1) | carry; flag.C = carry; flag.N = 0; flag.Z = !r; flag.H = 0;   PC +=2;}, "RLC " #r }
#define RRC_n(r)        { [](){ TC += 8; char carry = (r & 0x01); r = (r >> 1) | carry<<7; flag.C = carry; flag.N = 0; flag.Z = !r; flag.H = 0;   PC +=2;}, "RRC " #r }
#define RL_n(r)         { [](){ TC += 8; char carry = (r & 0x80)>>7; r = (r << 1) | flag.C; flag.C = carry; flag.N = 0; flag.Z = !r; flag.H = 0;   PC +=2;}, "RL " #r }
#define RR_n(r)         { [](){ TC += 8; char carry = (r & 0x01); r = (r >> 1) | carry<<7; flag.C = carry; flag.N = 0; flag.Z = !r; flag.H = 0;   PC +=2;}, "RR " #r }
#define SLA_n(r)        { [](){ TC += 8; flag.C = (r & 0x80)>>7; r = (r << 1); flag.N = 0; flag.Z = !r; flag.H = 0;   PC +=2;}, "SLA " #r }
#define SRA_n(r)        { [](){ TC += 8; char carry = (r & 0x01); r = (r >> 1) | (r & 0x80); flag.C = carry; flag.N = 0; flag.Z = !r; flag.H = 0;   PC +=2;}, "SRA " #r }
#define SWAP(r)         { [](){ TC += 8; r = r>>4 | r<<4; flag.C = 0; flag.N = 0; flag.Z = !r; flag.H = 0;   PC +=2;}, "SWAP " #r }
#define SRL(r)          { [](){ TC += 8; char carry = (r & 0x01); r = (r >> 1); flag.C = carry; flag.N = 0; flag.Z = !r; flag.H = 0;   PC +=2;}, "SRL " #r }
#define BIT(b,r)        { [](){ TC += 8; flag.Z = !(r & (0x01 << b)); flag.N = 0; flag.H = 1; PC +=2;}, "BIT " #b ", " #r};
...
void Glouboy::init()
{
    ...
    opcode[0x36] = LD_r_n(ram[hl],12);
	opcode[0x0E] = LD_r_n(reg_c,  8);
	opcode[0x1E] = LD_r_n(reg_e,  8);
	opcode[0x2E] = LD_r_n(reg_l,  8);
	opcode[0x3E] = LD_r_n(reg_a,  8);
	
	opcode[0x07] = { []() { /* RLCA    */ TC += 4;  char carry = (reg_a & 128) > 0; reg_a = (reg_a << 1) | carry;     flag.C = carry; flag.N = 0; flag.Z = 0; flag.H = 0;   PC += 1; } , "RLCA" };
	opcode[0x17] = { []() { /* RLA    */  TC += 4;  char carry = (reg_a & 128) > 0; reg_a = (reg_a << 1) | flag.C;   flag.C = carry; flag.N = 0; flag.Z = 0; flag.H = 0;   PC += 1; } , "RLA" };
	opcode[0x0F] = { []() { /* RRCA    */ TC += 4;  char carry = (reg_a & 1); reg_a = (reg_a >> 1) | (carry << 7);  flag.C = carry; flag.N = 0; flag.Z = 0; flag.H = 0;   PC += 1; }, "RRCA" };
	opcode[0x1F] = { []() { /* RRA    */  TC += 4;  char carry = (reg_a & 1); reg_a = (reg_a >> 1) | (flag.C << 7); flag.C = carry; flag.N = 0; flag.Z = 0; flag.H = 0;   PC += 1; }, "RRA" };
    ...
    opcode[0x2A] = LD_r_ss(reg_a,ram[hl++]);
    opcode[0x3A] = LD_r_ss(reg_a,ram[hl--]);
	...
	opcode[0xE2] = /* LD (C) A   */{ []() { TC += 8; ram[0xff + reg_c] = reg_a; PC += 1; }, " LD (C) A" };
	opcode[0xF2] = /* LD  A (C)  */{ []() { TC += 8; reg_a = ram[0xff + reg_c]; PC += 1; }, "LD  A (C)" };
	opcode[0xE0] = /* LDH (a8) A */{ []() { TC += 12; ram[0xff + rom[PC + 1]] = reg_a; PC += 2; }, "LDH (0x%02x) A" };
	opcode[0xF0] = /* LDH  A (a8)*/{ []() { TC += 12; reg_a = ram[0xff + rom[PC + 1]]; PC += 2; }, "LDH  A (0x%02x)" };
    
	opcode[0xEA] = /* LD (a16) A */{ []() { TC += 16; ram[rom[PC + 1] | (rom[PC + 2] << 8)] = reg_a; PC += 3; }, "LD (0x%02x%02x) A" };
	...
	opcode[0xF8] = /* LD HL,SP+r8*/{ []() { TC += 12; flag.N = 0; flag.Z = 0;
	...
	opcode[0xE8] = /* ADD SP,r8*/{ []() { TC += 16; flag.N = 0; flag.Z = 0;
										  int x = (signed char)(rom[PC + 1]);
										  int result = SP + x;
										  int carrybits = x ^ SP ^ result;
										  update_carry();
										  SP = result;
										  PC += 2; }, "ADD SP,0x%02x" };
	opcode[0xC2] = JP_X_A16(!flag.Z);
	opcode[0xD2] = JP_X_A16(!flag.C);
	opcode[0xCA] = JP_X_A16(flag.Z);
	opcode[0xDA] = JP_X_A16(flag.C);
	
	opcode[0xCD] = CALL_X_A16(true);
	opcode[0xCC] = CALL_X_A16(flag.Z);
	opcode[0xDC] = CALL_X_A16(flag.C);
	opcode[0xC4] = CALL_X_A16(!flag.Z);
	opcode[0xD4] = CALL_X_A16(!flag.C);
	...
	opcode[0xC8] = RET_X(flag.Z);
	opcode[0xD8] = RET_X(flag.C);
	opcode[0xC0] = RET_X(!flag.Z);
	opcode[0xD0] = RET_X(!flag.C);
	...
	opcode[0xFE] = CP_r(rom[++PC],8);
    
	opcode[0x18] = { []() { /* JR r8   */  TC += 12;  PC += (signed char)(rom[PC + 1]) + 2; }, "JR 0x%02x" };
    opcode[0x20] = JR_X_r8(!flag.Z);
    opcode[0x30] = JR_X_r8(!flag.C);
    opcode[0x28] = JR_X_r8(flag.Z);
    opcode[0x38] = JR_X_r8(flag.C);
	opcode[0x2F] = { []() { /* CPL */ TC += 4; reg_a ^= 0xFF; flag.N = 1; flag.H = 1; PC += 1; }, "CPL" };
	opcode[0x3F] = { [](){ /* CCF */ TC += 4; flag.C ^= 1; flag.N = 0; flag.H = 0; PC += 1;}, "CCF" };
	opcode[0x37] = { [](){ /* SCF */ TC += 4; flag.C = 1; flag.N = 0; flag.H = 0; PC += 1;}, "SCF" };
	opcode[0x27] = { [](){ /* DAA */ 
		TC += 4; 
		if (!flag.N)
		{
			if (flag.H || ((reg_a & 0xF) > 9)) { reg_a += 0x06; flag.C = 0; }
			if (flag.C ||  (reg_a > 0x9F))     { reg_a += 0x60; flag.C = 1; }
		}
		else
		{
			if (flag.H) { reg_a -= 0x06; flag.C = 0; }
			if (flag.C) { reg_a -= 0x60; flag.C = 1; }
		}
	...
	opcode[0xCB] = { []() { char op = rom[PC + 1]; CBopcode[op].funct(); if ((op & 0x07) == 6) { TC += 8; } }, "prefixe CB" };
...
void Glouboy::update()
{
	mem_edit_1.DrawWindow("Memory Editor", (unsigned char*)rom, romSize, PC); // create a window and draw memory editor (if you already have a window, use DrawContents())
//    unsigned char instruction = rom[PC];
	static bool hexa = true;
	ImGui::PushItemWidth(120);
	ImGui::Checkbox("display hexa", &hexa);
	int inputflag = hexa ? ImGuiInputTextFlags_CharsHexadecimal : 0;
	ImGui::InputInt("SP", &SP, 1, 0, inputflag);
	ImGui::InputInt("PC", &PC, 1, 0, inputflag);
	...
	ImGui::Text("Flags Z:%x C:%x H:%x N:%x", flag.Z ? 1 : 0, flag.C ? 1 : 0, flag.H ? 1 : 0, flag.N ? 1 : 0);
	if (ImGui::Button("Z"))
	{
		flag.Z ^= 1;
	} ImGui::SameLine();
	...
	{
		ImGui::Text("A:0x%x B:0x%x C:0x%x D:0x%x E:0x%x F:0x%x H:0x%x L:0x%x", reg_a, reg_b, reg_c, reg_d, reg_e, reg_f, reg_h, reg_l);
		ImGui::Text("AF:0x%x BC:0x%x DE:0x%x HL:0x%x", af, bc, de, hl);
...

	{
		...
		ImGui::Text("Current instruction : %s", instructionLabel);
	}
